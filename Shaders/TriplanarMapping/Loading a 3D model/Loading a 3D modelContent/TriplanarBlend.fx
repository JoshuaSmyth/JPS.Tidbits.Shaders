...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
    ...
    output.Sample1 = worldSample.yz * textureScale + sign(input.Position.x) * 0.25f; // The sign part removes the symetry
    output.Sample2 = worldSample.zx * textureScale;
    output.Sample3 = worldSample.xy * textureScale + sign(input.Position.z) * 0.25f; // The sign part removes the symetry
...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
   ...
   // Side A
   float4 col1 = tex2D(ColorMapSampler, input.Sample1)* 0.01 + float4(1.0,0.0,0.0,1.0);
   
   // Ceiling
   float4 col2 = tex2D(ColorMapSampler, input.Sample2) * 0.01 + float4(0.0,1.0,0.0,1.0);
   
   // Side B
   float4 col3 = tex2D(ColorMapSampler, input.Sample3) * 0.01 + float4(0.0,0.0,1.0,1.0);
